...
THROTTLE_LIMIT = 120  # Mirrors DEFAULT_THROTTLE_RATES['booking'] in settings.py.
...
class ThrottleTests(ThrottleResetMixin, TestCase):
    ...
    def test_confirm_shares_the_booking_budget(self):
        # Documents current behaviour rather than endorsing it: create and confirm use
        # the same throttle scope, so an office that produced 120 bookings in an hour
        # then cannot confirm from that address -- and confirm is the one request a buyer
        # who has already transferred must be able to make. If this test starts failing
        # because confirm got its own scope, delete it; that is the better design.
        responses = self._post_from_one_ip(THROTTLE_LIMIT)
        reference = responses[0].data['reference']
        self.assertEqual(self.client.post(confirm_url(reference)).status_code, 429)
        # From any other address the same confirm goes straight through.
        self.assertEqual(
            self.client.post(confirm_url(reference), REMOTE_ADDR=ip_for(1)).status_code,
            200,
        )
